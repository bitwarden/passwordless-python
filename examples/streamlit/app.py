...
st.write("[Documentation](https://docs.passwordless.dev) - [Get your API keys]("
         "https://admin.passwordless.dev/signup)")

if "passwordless_api_key" not in st.secrets or "passwordless_api_secret" not in st.secrets \
        or "passwordless_api_url" not in st.secrets \
        or st.secrets["passwordless_api_key"] == "" or st.secrets["passwordless_api_secret"] == "" \
        or st.secrets["passwordless_api_url"] == "":
    st.write("To run this example you must first specify yor API and API Secret in _.streamlit/secrets.toml_ file.")
